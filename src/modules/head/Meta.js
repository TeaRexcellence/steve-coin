  ...
  return (
    <>
      <Head>
        ...
        {/* Is a home page */}
        {router.pathname === '/' ? (<title>{`${Config.site_name} | ${props.metaTitle}`}</title>) : ('')}
        {/* Is not a post or the home page */}
        {router.pathname.indexOf("post") == -1 && router.pathname !== '/' ? (<title>{`${Config.site_name} | ${props.title}`}</title>) : ('')}
        ...
        <meta key="twitter:site" name="twitter:site" content="@SteveSeagullEt" />
        <meta key="twitter:creator" name="twitter:creator" content="@SteveSeagullEt" />
